...
11 盛最多水的容器
    题目：给定 n （n>=2）个非负整数 a1，a2，...，an，每个数代表坐标中的一个点 (i, ai) 。在坐标内画 n 条垂直线，
...
14 最长公共前缀
    ...
    如果不存在公共前缀，返回空字符串 ""
15 三数之和
    题目：给定一个包含 n 个整数的数组 nums，判断 nums 中是否存在三个元素 a，b，c ，
...
49 字母异位词分组
    题目：给定一个字符串数组，将字母异位词组合在一起。字母异位词指字母相同，但排列不同的字符
...
80 删除排序数组中的重复项 II
    题目：给定一个排序数组，你需要在原地删除重复出现的元素，使得每个元素最多出现两次，返回移除后数组的新长度。
        不要使用额外的数组空间，你必须在原地修改输入数组并在使用 O(1) 额外空间的条件下完成。
81 搜索旋转排序数组 II
    题目：编写一个函数来判断给定的目标值是否存在于数组中。若存在返回 true，否则返回 false
82 删除排序链表中的重复元素 II
...
91 解码方法
92 翻转链表II
    题目：反转从位置 m 到 n 的链表。请使用一趟扫描完成反转。1 ≤ m ≤ n ≤ 链表长度
    提示：创建虚拟头结点
...
149 直线上最多的点数
    ...
    提示：查找表
150 逆波兰表达式求值
    题目：有效的运算符包括 +, -, *, / 。每个运算对象可以是整数，也可以是另一个逆波兰表达式
    提示：栈
...
202 快乐数
    题目：一个“快乐数”定义为：对于一个正整数，每一次将该数替换为它每个位置上的数字的平方和，然后重复
...
205 同构字符串
    题目：给定两个字符串 s 和 t，判断它们是否是同构的。
        如果 s 中的字符可以被替换得到 t ，那么这两个字符串是同构的。所有出现的字符都必须用另一个字符替换，
        同时保留字符的顺序。两个字符不能映射到同一个字符上，但字符可以映射自己本身。
    提示：enumerate,新建一个字典，pattern元素为key,str为value 同290
206 翻转链表
    题目：反转一个单链表
...
215 数组中的第K个最大元素
    ...
    请注意，你需要找的是数组排序后的第 k 个最大的元素，而不是第 k 个不同的元素。不要去重
...
219 存在重复的元素2
    ...
    提示：查找表
220 存在重复的元素3
    题目：给定一个整数数组，判断数组中是否有两个不同的索引 i 和 j，
...
242 有效的字母异位词
    ...
    例子：输入: s = "anagram", t = "nagaram"     输出: true
    提示：字典统计次数
...
290 单词模式
    ...
    提示：enumerate,新建一个字典，pattern元素为key,str为value
...
349 两个数组的交集
    题目：给定两个数组，编写一个函数来计算它们的交集。输出结果中的每个元素一定是唯一的。
            我们可以不考虑输出结果的顺序
        示例 1:
    ...
    提示：集合set交集
350 两个数组的交集 II
    题目：给定两个数组，编写一个函数来计算它们的交集。输出结果中每个元素出现的次数，应与元素在两个数组中出现的次数一致。
        我们可以不考虑输出结果的顺序。
        示例 1:
